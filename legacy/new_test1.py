...
def get_result_prompt(query):
    ...
    """
    url = f"http://localhost:3335/api/search?query={query}&knowledgeLabel=1&reset=true"
...
def predict(query, ground_truth, model, dataset):
    """
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        for instance in tqdm.tqdm(instances):
            ...
            try:
                ...
                ans = instance['answer']
                # 不再处理 docs、file_path，只做推理
                label, prediction, factlabel = predict(query, ans, model, args.dataset)
